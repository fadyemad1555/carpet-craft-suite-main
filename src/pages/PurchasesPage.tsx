        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          ...
          <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
            ...
            <form onSubmit={handleSave} className="space-y-3 mt-2">
              <div><Label>المورد</Label><Input name="supplier" required placeholder="اسم المورد" /></div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><Label>المبلغ المدفوع</Label><Input name="paid" type="number" defaultValue={0} required /></div>
              ...
              <tbody>
                {filtered.map((inv) => (
                  <tr key={inv.id} className="border-b border-border/50 last:border-0 hover:bg-muted/30 transition-colors">
                    ...
                    <td className="p-3"><Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => setViewInvoice(inv)}><Eye className="h-3.5 w-3.5" /></Button></td>
                  </tr>
                ))}
              </tbody>
            </table>
