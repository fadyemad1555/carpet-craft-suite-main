...
export default function SalesPage() {
  ...
  const handleAddItem = () => setItems([...items, { productName: "", quantity: 1, price: 0 }]);
          ...
          <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
            ...
            <form onSubmit={handleSave} className="space-y-3 mt-2">
              <div className="space-y-2">
                ...
                {items.map((item, idx) => (
                  <div key={idx} className="flex gap-2 items-end">
                    ...
                    {items.length > 1 && <Button type="button" variant="ghost" size="icon" className="h-10 w-10 text-destructive shrink-0" onClick={() => handleRemoveItem(idx)}><Trash2 className="h-4 w-4" /></Button>}
                  </div>
                ))}
                <Button type="button" variant="outline" size="sm" onClick={handleAddItem}>+ إضافة منتج</Button>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div><Label>الخصم</Label><Input name="discount" type="number" defaultValue={0} /></div>
              ...
              <div>
                ...
                <Input name="paid" type="number" defaultValue={0} required />
              </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead><tr className="border-b bg-muted/50">
                ...
                <th className="p-3 text-right font-medium text-muted-foreground">المنتجات</th>
              ...
              <tbody>
                {filtered.map((inv) => (
                  <tr key={inv.id} className="border-b border-border/50 last:border-0 hover:bg-muted/30 transition-colors">
                    ...
                    <td className="p-3 text-muted-foreground">{inv.items.map((i) => i.productName).join(", ")}</td>
                    <td className="p-3 font-semibold text-accent">{inv.total} ج.م</td>
                    <td className="p-3"><Badge variant={inv.paid >= inv.total ? "secondary" : "destructive"}>{inv.paid >= inv.total ? "مدفوعة" : "غير مكتملة"}</Badge></td>
                    <td className="p-3">
